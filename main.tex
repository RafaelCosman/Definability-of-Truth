...
We wish to construct a probabilistic language which can model its own sentences' probabilities, which we take as a continuous analogue to the truth values of sentences.
...
\label{refl-refl}
%%fakesubsection
If we think of a reflective $\PP$ as one that can 'model' itself, we might want more than for $P$ to just approximate $\PP$ arbitrarily well. We might want $\PP$ to be confident that \emph{$P$ obeys the same principles as $\PP$}, namely the Reflection principle and Gaifman coherence. We will show that unfortunately any coherent $\PP$s that assign non-zero probability to $P$ exhibiting reflection are \emph{necessarily not themselves reflective}. This result intuitively means that reflective $\PP$s cannot model their own reflection and $\PP$s that think they model their own reflection are not actually reflective.
...
We would also like $\PP$ to assign probability $1$ to its own Gaifman coherence. Here, the situation is much better. In fact, we can prove that there exist $\PP$'s which do exactly this.
...
\end{proof}
We would like to combine this result with the inner reflection principle, but this is impossible. No $\PP$ can assign positive probability to both its coherence and its inner reflection principle.
\begin{proposition}
