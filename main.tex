...
\maketitle
\begin{abstract}
We prove that there exist coherent probability distributions $\PP$ over sentences that satisfy a reflection principle, namely, that any $\PP$ assigns $0$ probability to any sentence saying that an inner function symbol $\Pq{\varphi}$ differs by more than an infinitesimal amount from the value of $\PP(\varphi)$. An analogous reflection principle is proved for expectations. We proceed to show that such $\PP$s are unfortunately not meta-reflective, that is, they assign zero probability to an inner language version of the reflection principle. We prove that these probability distributions are necessarily supported on nonstandard models.
...
A central notion in metamathematics is the \emph{truth} of a sentence. To express this notion within a theory, we introduce a predicate $\True$ which acts on quoted sentences $\q{\varphi}$ and returns their truth value $\Tq{\varphi}$ (where $\q{\varphi}$ is a representation of $\varphi$ within the theory, for example its G\"{o}del number).
...
Hajek has shown that an analogue of this is true in the context of fuzzy logic\cite{hajek98} and similar paradoxes arise in introspective probabilistic logics \cite{campbell14}.
We construct a probabilistic predicate which, in some sense, can talk about itself. We then prove that it is consistent and does not fall prey to the standard paradoxes. It turns out that our construction is very extensible, allowing us to, for example, talk about expectations coherently.

Our work relies heavily on previous work done by Gaifman. In \cite{gaifman64}, Gaifman introduces a set of coherence conditions for probabilities $\PP$ defined over languages containing arithmetic. We will refer to these conditions as the Gaifman conditions:
\begin{enumerate}
\item If $\vdash \varphi$ and $\vdash \psi$ then $\PP(\varphi)=\PP(\psi)$
\item If $\vdash \neg (\varphi \wedge \phi)$ then $\PP(\varphi \vee \psi) = \PP(\varphi) + \PP(\psi)$
\end{enumerate}
We add to these the condition that $\forall\varphi:\PP(\varphi)\in[0,1]$.

We later show that these are equivalent to a measure over complete theories extending some base theory.

In general, our work can be seen as attacking the problem of logical uncertainty, building off of \cite{gaifman04}. It is easy to interpret the present paper in the context of frameworks of autonomous, probabilistic agents, such as those studied by \cite{hutter13} and \cite{demski12}.
The way that we handle the version of the liar's paradox that arises in our system is similar to that in \cite{hajek98}, especially since both approaches necessarily require the use of nonstandard models. Replacing the biconditional with a conditional in our reflection principle mirrors \cite{scharp13}'s distinction between ascending and descending truth.

Most importantly, our work has to do with self-referential probabilities. We build a system very similar in purpose to those constructed by \cite{leitgeb08}, \cite{leitgeb12b}, and \cite{campbell14}. We deal with similar problems to those presented in \cite{leitgeb12a}.

We begin in section~\ref{refl-cons} by defining a reflection principle and then proving that it is consistent. In section~\ref{refl-refl} we explore the implications of this, showing that our principle satisfies weak versions of some meta-reflective desiderata. We also prove some impossibility results. In section~\ref{refl-exp}, we derive a version of our reflection principle for expectations. Along the way, we prove an alternate characterization of coherence, which makes many new tools available to us. In section~\ref{nonstandard-models}, we show that our measures are supported on nonstandard models; we demonstrate a result that tells why this is worrisome. We also prove the existence of finitely additive measure that fulfills most of our desiderata and is supported on standard theories. In section~\ref{further-work}, we argue for weakening the reflection principle and mention some open problems.
...
Take some consistent theory $T$ over some language $L$. Call $L'$ the extension of $L$ by a function symbol $P$. Take $\mathbb{P}$ to be Gaifman coherent over $T$. Intuitively, we want $P$ to be able to estimate $\mathbb{P}$, that is, to \emph{reflect} $\mathbb{P}$.
...
\forall \varphi \in L' \; \forall a, b \in \mathbb{Q} : \of{a < \PP(\varphi) < b} \iff \PP(a < \Pq{\varphi} < b) = 1
\end{equation}
...
\forall \varphi \in L' \; \forall a, b \in \mathbb{Q} : \left(a < \Po{\varphi} < b\right) \Longrightarrow \Po{a < \Pq{\varphi} < b} = 1 \\
\end{equation}
We say that $\PP$ is \emph{reflectively consistent} if it satisfies this condition.
Note that the reverse implication is not true; however, by taking the contrapositive, it is easy to show that condition~\ref{reflection} is equivalent to
\begin{equation}\label{reflection-converse}
\forall \varphi \in L' \; \forall a, b \in \mathbb{Q} : \left(a \leq \Po{\varphi} \leq b\right) \Longleftarrow \Po{a \leq \Pq{\varphi} \leq b} > 0
\end{equation}
Note that this is almost the reverse implication, except for the fact that it uses closed instead of open intervals.
...
We will now prove that $x$ is also in $\mathcal{A}$ by showing that $x$ must satisfy the  itions with respect to any $\varphi$. 
Fix a $\varphi$.
We take the sequence of reals $x_i(\varphi)$. Note that the conditions we give can be easily shown to be equivalent to the Gaifman conditions with the restriction of $\PP$ to the interval $[0, 1]$.
\begin{itemize}
...
\label{refl-refl}
%%fakesubsection
Interpreting a reflective $\mathbb{P}$ as an agent's belief state, we would like the agent to believe that the reflection principle holds as well as believe that $\mathbb{P}$ is Gaifman coherent. We show that only very weak forms of these desiderata can be satisfied.
\subsection{The Inner Reflection Principle}
The reflection principle is stated in the meta language, and is a 
...
\end{proposition}
\begin{proof}
Assume that there exists a $\PP$ which assigns probability 1 to
$$\forall a,b\in\mathbb{Q}\forall\varphi\in L',\PP(\Pq{\varphi}\in(a,b)\rightarrow\Pq{\Pq{\varphi}\in(a,b)}=1)=1$$.
Then, we have that 
\begin{align*}
\Po{a \leq \Pq{\varphi} \leq b} > 0
&\implies \Po{\Pq{a \leq \Pq{\varphi} \leq b} > 0} = 1 \\
&\implies \Po{a \leq \Pq{\varphi} \leq b} = 1
\end{align*}
...
$$\forall a,b,\varepsilon\in\mathbb{Q}\forall\varphi\in L',\PP(\Pq{\varphi}\in(a,b)\rightarrow\Pq{\Pq{\varphi}\in(a,b)}>1-\varepsilon)=1.$$
\end{proposition}
\begin{proof}
Assume that $\Pq{\varphi}\in (a,b)$. Then, $\PP(\varphi)\in(a,b)$.
By the contrapositive of the reflection principle, we have that
$$\PP(\Pq{\varphi}\in(a,b))=1.$$
We can then derive 
$$1-\varepsilon<\PP(a<\Pq{\varphi}<b)<1+\varepsilon.$$
Applying the reflection principle, we get that we've proved
$$\Pq{\varphi}\in (a,b)\rightarrow\Pq{a<\Pq{\varphi}<b}>1-\varepsilon.$$
...
The other major property of coherent, reflective distributions is Gaifman coherence, which we would ideally have $\mathbb{P}$ assign probability $1$ to. Here, the situation is much better. In fact, we can prove that there exist $\mathbb{P}$'s which do exactly this.
...
$$\forall \varphi. (0 \leq \Pq{\varphi} \leq 1) \wedge (\varphi \rightarrow \Pq{\varphi}=1) \wedge (\forall \psi. \Pq{\varphi} = \Pq{\varphi \wedge \psi} + \Pq{\varphi \wedge \neg \psi})$$
\begin{proposition}
...
\end{proof}
We would like to combine this result with the inner reflection principle, but it turns out that this is impossible. No $\mathbb{P}$ can assign positive probability to both its coherence and its inner reflection principle.
\begin{proposition}
There does not exist a coherent, reflective $\mathbb{P}$ such that
$$\forall a,b,\varepsilon\in\mathbb{Q}\forall\varphi\in L',\PP(Coh(P)\wedge(\Pq{\varphi}\in[a,b]\rightarrow\Pq{\Pq{\varphi}\in(a,b))}=1)>\varepsilon$$
\end{proposition}
\begin{proof}
...
\begin{gather*}
\forall \varphi \in L' \; \forall a, b \in \mathbb{Q} \st \Po{\varphi \wedge \of{a < \Pq{\varphi} < b}} \leq b \Po{a \leq \Pq{\varphi} \leq b} \\
\forall \varphi \in L' \; \forall a, b \in \mathbb{Q} \st \Po{\varphi \wedge \of{a \leq \Pq{\varphi} \leq b}} \geq a \Po{a < \Pq{\varphi} < b}
\end{gather*}
...
We would like to thank Rafael Cosman for his help writing this paper.
