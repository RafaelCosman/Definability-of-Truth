...
\maketitle
\begin{abstract}
It is well known that truth predicates fail to liar's sentences.
One solution is to assign probabilities to sentences.
We develop a novel framework for probability distributions over sentences which achieves an important property called \emph{reflection}. 
Intuitively, this is the property that a probability distribution `believes' in itself.
We build an analogous framework for expectations and prove a similar property.
We also prove a negative result: such probability distributions cannot exhibit a property called inner reflection. 
Intuitively, while they believe in themselves, they cannot \emph{believe} that they believe in themselves.
\end{abstract}
...
In this work we will build a framework for reflective probability distributions. These distributions will be over a language including a symbol $P$ which will `reflect' the probabilities in the distribution. Just like how a truth predicate $True$ `reflects' the notion of truth in the theory,
...
Three-valued logics attempt to solve this issue by having an indeterminate value in addition to True and False. Sentences like $G$ are assigned the indeterminate value. These systems, however, are not satisfactory because they are unable to reach conclusions such as $G \vee \neg G$.

This work instead takes the approach of assigning probabilities to sentences. These probabilities are often thought of as 'degrees of belief'.
Much work has already been done in this vein, but there is a crucial property called reflection that has not been fully achieved. Leitgeb in \emph{From Type-Free Truth to Type Free Probability} constructs theories which exhibit attractive properties close to full reflection but they 
...
%TODO expand this bit greatly to talk more about our actual results. The intro has a lot about truth predicates and background, but little about our construction
%TODO maybe move the truth predicate discussion to the next subsection or cut it down
...
Others have also constructed probability distributions over sentences in order to combat this same issue. For example,
...
%TODO What do you mean contributes? The obvious interpretation is false.
to frameworks of autonomous, probabilistic agents such as those studied by Hutter and Demski
\cite{hutter13,demski12}.
%TODO I think that we need to either develop this part further or delete it 
...
\subsection{Structure of the Paper} %TODO [intro-sec-norms] check if a norm or not (note: Campbell-moore seems to do this, but she may not be representative)
...
and so, in this vein, we might consider the following reflection principle:
...
%%fakesubsection
Given a $\PP$,
if we think of $P$ as approximating $\PP$, then we would like $P$ to reflect not only the probability assignments of $\PP$, but also the its structure. Specifically, we would like $\PP$ to assign high probability to the statements that $P$ is reflective and Gaifman coherent. 

We will show that, unfortunately, any coherent $\PP$ that assigns non-zero probability to $P$ exhibiting reflection is \emph{necessarily non-reflective}. This result intuitively means that no reflective, coherent $\PP$ believes that $P$ is reflective; and, further, that any $\PP$ which does believe that its approximation is reflective is not itself reflective.
