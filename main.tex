...
Rather than constructing a \emph{truth} predicate and then assigning \emph{truth values} to sentences, we construct a \emph{probabilistic analogue} to a truth predicate and then assign \emph{probabilities} to sentences, including those that refer to the probabilities of other sentences. 
%TODO send to kenny to figure out if the inner outer language distinction is clear
Unlike the standard truth predicate construction, our analogue is consistent and does not fail to probabilistic generalizations of liar's sentences such as ``this sentence is unlikely''. 
...
%TODO decide whether to include other results here.
\end{abstract}
\setcounter{section}{-1}
\section{Introduction}

A truth predicate, if possible, would `reflect' truth in a theory:
...
%TODO Phrasing and cadence fixes here!!!
Let $\PP$ be a probability distribution over sentences. We think of $\PP$ as a probabilistic analogue to truth. $\PP$ has an opinion about each sentence in the language, even those sentences independent of the theory.
%TODO finish these edits from tablet markups. Also, so CLUNKY!
We will refer to a theory over which such a probability distribution $\PP$ is defined as a theory with a notion of \emph{outer probability} in addition to its normal notion of truth.

Let $P$ be a function symbol in the language which exhibits a property analogous to (\ref{truth-reflection}). We think of $P$ as a probabilistic analogue of a truth predicate. We will refer to functions $P$ that exhibit such a property as \emph{inner probability functions}.
...
Unfortunately, reflection principles of this strength fail to liar's sentences such as $\vp \iff \Pq{\vp} < 0.5$ (``This sentence is unlikely''). In order to achieve consistency, we need to weaken this reflection principle in some way. We take the approach of allowing $P$ to \emph{approximate} $\PP$ rather than equal $\PP$ exactly, as well as weakening the biconditional to a one directional implication.
...
\begin{definition} A \emph{coherent} probability distribution $\PP$ is one that satisfies the following:
\begin{center}
\begin{tabular}{l r}
...
Let $\PP$ be a coherent probability distribution over $L'$. We say that $\PP$ is \emph{reflective} iff it satisfies the following reflection principle:
...
we can obtain a \emph{coherent}, \emph{reflective} $\PP$.
...
\definitionsOfLsAndT
There exists a coherent $\PP$ which satisfies the reflection principle:
...
It is easy to see that $x$ is coherent and thus an element of $\mathcal{A}$.
...
Since every $x_i$ is coherent, this implies that $x_i(\vp)$ are all $1$. Thus they converge to $1$.
\item $\forall \psi: x(\vp) -x(\vp\wedge\psi)-x(\vp\wedge\neg\psi)=0$.
(Note that we have rewritten the condition to make this part clearer.)
Fix a $\psi$.
Since the $x_i$'s are all coherent, $\forall i\forall \psi: x_i(\vp) -x_i(\vp\wedge\psi)-x_i(\vp\wedge\neg\psi)=0.$
This is a sequence of zeroes, so it must converge to zero, so $x$ fulfills this condition.
\end{itemize}
Note that our choices of $\vp$ and $\psi$ were arbitrary. Thus the conditions hold for all $\vp$ and $\psi$. This implies that the limit $x$ is coherent and therefore $\mathcal{A}$ is closed.
Thus $\mathcal{A}$ is compact.

\item \emph{$\mathcal{A}$ is convex.}
Given any $x,z\in\mathcal{A}$, we will show that if $y$ is a linear combination of $x$ and $z$, then $y$ is coherent and therefore is in $\mathcal{A}$.
...
Therefore, convex combinations of coherent probability distributions are coherent.
...
Since $x$ is coherent, this theory is consistent.
Let $y$ assign $1$ to all statements in $T^*$ and $0$ to all statements not in $T^*$.
This $y$ satisfies $xRy$ and is coherent.
...
$$\vp \iff \Tq{\vp}.$$
If one had a working truth predicate, a natural next desidaratum is for the theory to \emph{know} that the truth predicate works as a truth predicate. Specifically, we might want the theory to prove
...
Analogously, now that we have proven that $P$ works like a truth predicate, we might additionally want $\PP$ to \emph{believe} that $P$ works like a truth predicate. That is, we would like $\PP$ to assign high probability to the statement that $P$ is reflective and coherent.

We will show that, unfortunately, no reflective, coherent~$\PP$ believes that $P$ is reflective and that any coherent~$\PP$ which does believe that $P$ is reflective is not itself reflective.
...
We will first prove that there exist \emph{coherent} inner reflective $\PP$s, but we then prove that these inner reflective $\PP$s cannot also be outer reflective.

\begin{proposition} \emph{Existence of an Inner Reflective $\PP$.}
\definitionsOfLsAndT Then there exists a coherent $\PP$ which satisfies the inner reflection principle:
...
\definitionsOfLsAndT
Then there exists no coherent $\PP$ which assigns probability one to every sentence in $T$ and satisfies both the inner and outer reflection principles. In fact, any coherent, reflective $\PP$ must assign probability $0$ to inner reflection.
\end{proposition}
\begin{proof}
...
Every coherent, reflective $\PP$ satisfies
...
We would also like $\PP$ to assign probability $1$ to $P$ being coherent. Here, the situation is much better. We will prove that there exist $\PP$'s which do exactly this.
Let $Coh(P)$ be the sentence stating that $P$ is coherent. Specifically, $Coh(P)$ is
...
%TODO rafael and which assigns probability~1 to the proposition that $P$ is a coherent probability distribution which itself assigns probability~1 to the same proposition. 
...
\end{definition}
Intuitively, $P$ is a three-valued relation which approximates some function $f$ arbitrarily well. We imagine $(\q{\vp},a,b)\in P$ to mean that $a<f(\vp)<b$, with $f$ being the function that we require to exist above. 
