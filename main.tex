...
We wish to construct a probabilistic language which can represent its own sentences' probabilities, which we take as a continuous analogue to the truth values of sentences.
...
In this work, we show that a probabilistic analogue succeeds where the truth predicate failed. We prove that our construction is consistent and does not fall prey to the standard paradoxes.
...
In section~\ref{further-work}, we argue for weakening the reflection principle and mention some open problems. %XXX KAYA in this sentence, should it say "finitely additive measure" instead of "additive measure"?
...
Usually, a language has sentences which are either true or false. Having introduced the concept of Gaifman coherence, we can describe languages in which sentences are assigned probabilities as opposed to truth values. Let $\PP(\vp)$ refer to the probability of a sentence $\vp$. For example, $\PP(\vp)=0$ means that the probability of $\vp$ is $0$. Note that $\PP$ is a function from the language to the real interval $[0,1]$, \emph{not} a symbol in the language.
...
We will now introduce the probabalistic analogue to the truth predicate $True$: A function symbol in the language, $P$, which approximates the values of $\PP$. 

Just like how $True$ attempted to satisfy

\begin{equation}
\vp \iff True(\q{\vp}),
\end{equation}

We might want to have the function symbol $P$ satisfy
...
Let $(a, b)$ be any open interval containing $\Po{\vp}$.
...
But this unfortunately also falls to the same liar's paradox ($\vp \iff \Pq{\vp} < 1$). The issue here turns out to be the bi-implication. Replacing it with implication yields a consistent reflection principle.
...
Let $\mathbb{P}$ be a Gaifman coherent probability distribution over $T$. We say that $\PP$ is \emph{reflectively consistent} if it satisfies
...
%XXX FIX to not refer to agents (req. intro rewritten)

The intuition behind the name ``reflective consistency''
is that there is a revision operation, intuitively called 'reflection', such that reflectively consistent $\PP$s are exactly those $\PP$s that do not change upon reflection.
...
by constructing the revision operation and appealing
to Kakutani's fixed point theorem.
\begin{theorem} \emph{Consistency of the Reflection Principle}: 
Let $T$ be a consistent theory over a language $L$. Extend $L$ by a function symbol $P$ to get the language $L'$. There exists a Gaifman coherent $\mathbb{P}$ which assigns probability one to every sentence in $T$ and satisfies the reflection principle:
...
$$xRy \iff \forall x, y \in \mathbb{Q}: \inOpen{x(\vp)} \rightarrow y(\inOpen{P(\vp)})=1$$
...
Let $\mathbb{P}(\vp) = 1$ for all $\vp$ in $T^{*}$ and $\mathbb{P}(\vp) = 0$ otherwise.
...
$$\forall a,b,\ varepsilon>0\in\mathbb{Q}\forall\vp\in L':\PP(\Pq{\vp}\in(a,b)\rightarrow\Pq{\Pq{\vp}\in(a,b)}>1-\varepsilon)=1.$$
\end{proposition}
\begin{proof}
Assume that $\Pq{\vp}\in (a,b)$ is a theorem.
%XXX Then, $\PP(\vp)\in(a,b)$.
