...
\newcommand{\definitionsOfLsAndTFirst}{Let $T$ be a consistent theory over some language $L$ admitting a G\"odel numbering.
...
Rather than constructing a \emph{truth} predicate and then assigning \emph{truth values} to sentences, we construct a \emph{probabilistic analogue} to a truth predicate and then assign \emph{probabilities} to sentences. 
Unlike the standard truth predicate construction, our analogue is consistent and handles probabilistic generalizations of liar's sentences such as ``this sentence is unlikely''. 
...
To model these in a mathematical theory, we would like a predicate $True$ such that ``X is true'' could be written $True(X)$. Such a predicate, if possible, would `reflect' truth in a theory\footnote{
...
We notate our probabilistic truth predicate with $P$, a function symbol in the language.
We want $P$ to reflect $\PP$, probabilistic truth in the theory.
...
In section~\ref{refl-exp} we derive a reflection principle for expectations.
...
Analogously to the proposed predicate $True$ reflecting the notion of truth in the theory, we want a function $P$, in the language, to reflect the outer probability distribution $\PP$.
...
\end{equation}
But, this again falls to the same liar's sentence. However, combining the two approaches yields a one directional, approximate reflection principle which we will prove to be consistent.
...
\end{equation}
Note the reverse implication, the closed intervals, and a greater than zero rather than an equal to one. We use this \emph{reverse reflection principle} in some of the proofs in section~\ref{refl-refl}.
...
\end{theorem}
\begin{proof}
Take the set $\mathcal{A}$ of all coherent probability distributions over $L'$ which assign probability one to all sentences in $T$ and consider it a subset of $[0,1]^{L'}$ with the product topology.
...
Thus we have shown that - for every consistent theory $T$ over $L$ - there exists a coherent, reflective $\PP$ over $L'$.
...
\subsection{The Inner Reflection Principle and Inner Coherence}
The reflection principle is stated in the metalanguage, and is a 
\emph{statement which is true about $\PP$}, not a sentence in the language.
...
\end{proposition}
\begin{proof}
We proceed with a double proof by contradiction.
Suppose there existed a coherent, reflective $\PP$ which assigns nonzero probability to inner reflection.
Let $\vp$ be the following liar's sentence:
$$\vp\leftrightarrow\Pq{\vp}<1$$
We first show that $\PP(\vp)<1$.
Suppose $\PP(\vp)=1.$
Then, $\PP(\Pq{\vp}<1)=1$.
Since $\PP$ assigns nonzero probability to inner reflection, we have that $\PP(\Pq{\Pq{\vp}<1}=1)>0$, or, equivalently, $\PP(\Pq{\vp}=1)>0$.
The sentence $\vp$ says that $\Pq{\vp}
<1$, which contradicts the statement $\Pq{\vp}=1$.
Therefore,
$T\vdash \neg(\vp\wedge\Pq{\vp}=1)$.
Thus by the coherence of $\PP$, we have that $\PP(\vp\vee\Pq{\vp}<1)>1$.
This contradicts $\PP(\vp)\in[0,1]$.

Therefore, $\PP(\vp)<1$. 
By the outer reflection principle, $\PP(\Pq{\vp}<1)=1$. Coherence of $\PP$ implies that $\PP(\Pq{\vp}<1)=\PP(\vp)$, a contradiction.
\end{proof}
...
Note that this is weaker entirely due to the internal `$>1-\epsilon$' in place of the usual `$=1$'. Also note the closed interval in place of an open interval in the antecedent.
\end{proposition}
\begin{proof}
Assume that $\PP(\Pq{\vp}\in(a,b))=1$. Then, $\PP(\Pq{\vp}\in(a,b))>1-\varepsilon$.
Applying the reflection principle, we get that 
$$\PP(\Pq{\inOpen{\Pq{\vp}}}>1-\varepsilon)=1.$$
Therefore,
$$\PP(\Pq{\vp}\in (a,b)\rightarrow\Pq{\inOpen{\Pq{\vp}}}>1-\varepsilon)=1.$$

Say that $\PP(\Pq{\vp}\in(a,b))<1$.


$$\PP(\Pq{\vp}\not\in (a,b)\vee\Pq{\inClosed{\Pq{\vp}}}>1-\varepsilon)=1.$$
$$\PP(\Pq{\vp}\in (a,b))=1$$
$$\PP(\Pq{\vp}\in [0,a]\cup[b,1]\vee\Pq{\inClosed{\Pq{\vp}}}>1-\varepsilon)=1.$$
$$=\PP(\Pq{\vp}\in [0,a]\cup[b,1])+\PP(\Pq{\inClosed{\Pq{\vp}}}>1-\varepsilon)+\PP(\neg(\Pq{\vp}\in [0,a]\cup[b,1]\wedge\Pq{\inClosed{\Pq{\vp}}}>1-\varepsilon)).$$

---OLD
%TODO MATH 2 fix this proof
...
There exist much weaker versions of the inner reflection principle which can be proven consistent with the outer reflection principle.
These weak inner reflection principles are not very interesting, and the proofs are easy, so we leave them out.

Though we can't have full inner reflection, we may at the very least want inner coherence, that is, we might want $\PP$ to assign probability $1$ to $P$ being coherent.
...
\end{proposition}
\begin{proof}
We prove this by modifying the proof of consistency of the reflection principle (theorem~\ref{ref-cons-pf}).

Redefine $\cA$ as the space of all coherent probability distributions over $L'$ which assign probability $1$ to $Coh(P)$. Since $T$ does not constrain $P$ in any way, and since the Gaifman conditions are consistent\cite{gaifman64}, $T+Coh(P)$ is consistent.
This implies that $\cA$ is nonempty, satisfying condition (2) of Kakutani's theorem.
Conditions (1), (3), (4), (5), (6), (7) follow with trivial modifications.
\end{proof}
