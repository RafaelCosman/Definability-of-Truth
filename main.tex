...
To model these in a mathematical theory, we would like a predicate $TRUE$ such that the English sentence from before could be written $TRUE(X)$.

A truth predicate $TRUE$, if possible, would `reflect' truth in a theory:
\begin{equation}\label{truth-reflection}
	\vp \iff \Tq{\vp}.
\end{equation}
But, given such a predicate, we could construct the liar's sentence $\vp\iff\neg\Tq{\vp}$ (``This sentence is false.''), which cannot be assigned a consistent truth value.
We conclude that truth predicates are inconsistent in classical two-valued logic; however, they may yet be salvaged by moving to a more expressive framework and relaxing some constraints on how they work.
We investigate an approach to this in the framework of probabilistic logic.

We want a probabilistic analogue of truth over our language, a function $\PP$ in the metalanguage which assigns probabilities to sentences in the language, even those sentences independent of the theory.
...
\end{equation}
Unfortunately, given this reflection principle, we can construct the liar's sentence $\vp \iff \Pq{\vp} < 0.5$ (``This sentence is unlikely''), which cannot be assigned a consistent probability.
We need to weaken this principle considerably before we arrive at a non-contradictory construction.

The approach we take is to convert the biconditional to an implication and to allow $P$ to \emph{approximate} $\PP$ rather than requiring it to equal $\PP$ exactly, arriving at the following reflection principle:
