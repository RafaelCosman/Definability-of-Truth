...
Rather than constructing a \emph{truth} predicate and then assigning \emph{truth values} to sentences, we construct a \emph{probabilistic analogue} to a truth predicate and then assign \emph{probabilities} to sentences. %XXX KAYA I recommend that we remove "including those that refer to the probabilities of other sentences." I find this to be confusing both because it breaks the parallel structure and also because it should be implied by the existence of a probabilistic analogue of a truth predicate. Right now it sounds like it might be some sort of property of our language independant of the probabilistic analogue of the truth predicate.
Unlike the standard truth predicate construction, our analogue is consistent. It handles probabilistic generalizations of liar's sentences such as ``this sentence is unlikely''. 
...
We work purely with inner and outer probability and outer truth because dispensing with an inner notion of truth lets us focus on the underlying difficulties of reflection in probabilistic logic.
We also extend our framework to talk about expectations.
