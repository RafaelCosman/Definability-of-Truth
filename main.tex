...
We wish to construct a probabilistic language which can represent its own sentences's probabilities, which we take as an analogue to the truth values of sentences.
If we wished to express a theory's truth values within the theory,
we might introduce a predicate $\True$ which acts on quoted sentences $\q{\varphi}$ and returns their truth value $\Tq{\varphi}$ (where $\q{\varphi}$ is a representation of $\varphi$ within the theory, for example its G\"{o}del number).

To confirm that the predicate is actually a correct representation, we would like it to satisfy a formal correctness property:
...
In this work, we show that a probabilistic analogue succeeds where the truth predicate failed, proving that our construction is consistent and does not fall prey to the standard paradoxes.
We also develop some additional results, showing that our construction extends to talking about expectations.
...
This falls immediately to the liar's paradox.
%XXX more here? May not be needed?
