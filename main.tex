...
Let $\PP$ be our probabilistic analogue for truth. That is, Let $\PP$ be a probability distribution over sentences such that if $\varphi$ is in our theory, then $\PP(\varphi) = 1$ and $\PP(\neg\varphi) = 0$. For all other sentences, $\PP$ should assign probabilities in some coherent way (we define a formal notion of coherence in the next subsection). $\PP$ can be thought of as having a probabilistic opinion about each sentence in the language, even if some such sentence is independent of the theory. We will refer to a theory over which such a $\PP$ is defined as a theory with a notion of \emph{outer probability}.
Let $P$ be a probabilistic analogue of a truth predicate. That is, we want $P$ to exhibit a property analogous to (\ref{truth-reflection}). We will refer to functions that exhibit such a property as \emph{inner probability functions}.
...
Unfortunately, reflection principles of this strength fail to liar's sentences such as $\vp \iff \Pq{\vp} < 1$. In order to achieve consistency, one must weaken this reflection principle in some way. We will take the approach of allowing $P$ to \emph{approximate} $\PP$ rather than equal $\PP$ exactly.
...
Our notion of outer probability follows these coherence conditions. Gaifman\cite{gaifman04} points to the fact that relaxations of coherence may lead to useful models of bounded reasoning. Given the interest in self-modifying reasoners (such as in XXX%TODO 
...
Just like how a truth predicate $True$ would allow a theory to refer to truth values of its own sentences, we would like a function symbol in the language which will allow our probabilistic logic to refer to the probabilities of its own sentences.
We will now introduce the probabilistic analogue of the truth predicate: A function symbol in the language, $P$, which approximates the values of $\PP$. This section will begin by showing why the obvious constructions fail before introducing our construction in definition~\ref{refl-def}.
...
But this falls immediately to a liar's sentence: consider $\vp \iff \Pq{\vp} < 1$.

We deal with the liar's paradox in a manner similar to Hajek \cite{hajek98}: we deny $P$ access to the exact values of $\PP$ but instead let $P$ have \emph{arbitrarily precise} information about $\PP$.
...
Let $\PP$ be a Gaifman coherent probability distribution over $L'$. We say that $\PP$ is \emph{reflective} iff it satisfies
...
is that there is a relation, intuitively called `belief', such that reflective $\PP$s are exactly those $\PP$s that `believe' in themselves.
...
Kakutani's fixed point theorem states that there exists $x \in \mathcal{A}$ such that $xRx$, given that the following hold:
\begin{enumerate}
...
\label{refl-refl}
%%fakesubsection
If we have a $P$ that functions like a truth predicate, then we might additionally want $\PP$ to \emph{believe} that $P$ functions like a truth predicate. Technically, we would like $\PP$ to assign high probability to the statement that $P$ is reflective and Gaifman coherent. 

We will show that, unfortunately, any coherent $\PP$ that assigns non-zero probability to $P$ exhibiting reflection is \emph{necessarily non-reflective}. This result intuitively means that no reflective, coherent $\PP$ believes that $P$ is reflective; and, further, that any $\PP$ which does believe that $P$ is reflective is not itself reflective.
...
We will prove that there exist \emph{coherent} inner reflective $\PP$s, but we then prove that these inner reflective $\PP$s cannot also be outer reflective.

\begin{proposition} \emph{Existence of an Inner Reflective $\PP$}
\definitionsOfLsAndT Then there exists a Gaifman coherent $\PP$ which assigns probability $1$ to every sentence in $T$ and satisfies the inner reflection principle:
...
\label{refl-exp}
%TODO check the exposition in this section (bottleneck: intro fix)
...
%TODO rafael change this proof to talk about x and y instead of ps
...
%TODO kaya can we use some word other than 'believe' here?
