...
\end{definition}
The proof of the following theorem is analogous to the proof of consistency of the reflection principle in a previous section. We define a function $f$ an then and then appeal to Kakutani's fixed point theorem to find a fixed point of $f$.
\begin{theorem}
...
\end{theorem}
\begin{proof}
Let $\cA$ be the range of the embedding $\iota$. Define $f : \cA\to\Pow{\cA}$, such that $\iota(\PP')\in f(\iota(\PP))$ if for every bounded definable value~$X$ and all $a,b\in\QQ$ such that $a<\EE[X]<b$, we have $\PP'[\dot a < E[X] < \dot b] = 1$.
...
\item \emph{$\mathcal{A}$ is convex.} Moreover,~$\cA$ is clearly convex, since~$\Delta(\cS_T)$ is convex and~$\iota$ clearly preserves convex combinations.
...
\item \emph{$R$ has a closed graph.} Clearly, the set $f(\iota(\PP))$ is closed, since it is the intersection of the closed set~$\cA$ with the closed sets $\{v\in\RR^\NN : v(n) = 1\}$, for every~$n$ that is the G\"odel number of a statement $\dot a < E[X] < \dot b$ that must be assigned probability~1.

\item \emph{$\forall x \exists y$ s.t. $xRy$.} $f(\iota(\PP))$ contains $\delta_{T(\cU,\PP)}$, the distribution that assigns probability~1 to~$T(\cU,\PP)$, the complete theory of the model~$(\cU,\PP)$.

\item \emph{$\forall x$ the set $\{y \in \mathcal{A}: xRy\}$ is convex.} Trivially
\end{enumerate}
