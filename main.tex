...
To model these in a mathematical theory, we would like a predicate $True$ such that the English sentence from before could be written $True(X)$. Such a predicate, if possible, would `reflect' truth in a theory:
...
Therefore, truth predicates are inconsistent in classical two-valued logic; however, they may yet be salvaged by moving to a more expressive framework and relaxing some constraints on how they work.
We investigate an approach to this using probabilistic logic.
...
Kakutani's fixed point theorem states that there exists $p \in \mathcal{A}$ such that $pRp$, given that the following hold\cite{kakutani41}:
\begin{enumerate}
...
\item \emph{$R$ has a closed graph (i.e. $R$ is a closed subset of $\cA^2$).}
