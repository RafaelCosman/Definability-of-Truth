...
%TODO send to kenny to figure out if the inner outer language distinction is clear
...
% Need to answer before continuing editing this section [Rafael: this is where the most important bottleneck is. Write answers in comments or just implement directly in the relevant TODO (they are labeled with refs of the form [example-label])]

%TODO [prob-log] What work has been done in probabilistic logic? What previous work do we motivate this from?

%TODO [explicit-norms] What is the norm for such an explicit discussion of previous work? Keep, cut down, expand?

%TODO [intro-sec-norms] Check the norms for splitting intros into such sections.

%TODO [deeper-refs] pick two papers and more deeply investigate their connection to our paper; write the paragraphs for the background section.

% TODO [prob-log] what work has been done in probabilistic logic before? Did you add papers to the list about this? Maybe there is a classic reference text that we could cite and be done (like Hajek is for fuzzy logic)?

%TODO Gaifman condition impossibility result from N.O. maybe add??

% add bits about probabilistic stuff here. Then draw analogy to truth values and stuff

% TODO add something about a revision sequence interpretation. Check if we can adapt the banrch sequence stuff from Leitgeb to prove consistency for this revision sequence. 

A truth predicate is intended to `reflect' truth in a theory:
%TODO dont like. Something about how it isnt the usual terminology and also how it isnt immediately clear what is going on.
\begin{equation}\label{truth-reflection}
	\vp \iff \Tq{\vp}.
\end{equation}

We use a probabilistic analogue to truth which will assign a probability to each sentence
and a probabilistic analogue to a truth predicate which will reflect those probabilities.

Let $\PP$ be our probabilistic analogue for truth. That is, Let $\PP$ be a probability distribution over sentences such that if $\varphi$ is in our theory, then $\PP(\varphi) = 1$ and $\PP(\neg\varphi) = 0$.
For all other sentences, $\PP$ will assign probabilities in a \coh way which we will define in the next subsection.
$\PP$ can be thought of as having a probabilistic opinion about each sentence in the language, even if some such sentence is independent of the theory.
% XXX KAYA can we give an example here?
For example, the G\"odel sentence of any theory is independent of the theory and so could be assigned probability anywhere from $0$ to $1$.
We will refer to a theory over which such a $\PP$ is defined as a theory with a notion of \emph{outer probability} in addition to its normal notion of truth.

Let $P$ be our probabilistic analogue of a truth predicate. That is, we want $P$ to exhibit a property analogous to (\ref{truth-reflection}). We will refer to functions that exhibit such a property as \emph{inner probability functions}.
...
Unfortunately, reflection principles of this strength fail to liar's sentences such as $\vp \iff \Pq{\vp} < 0.5$ (``This sentence is unlikely''). In order to achieve consistency, one must weaken this reflection principle in some way. We will take the approach of allowing $P$ to \emph{approximate} $\PP$ rather than equal $\PP$ exactly.

\subsection{Background and previous work}
%TODO [intro-sec-norms] Check the norms for splitting intros into such sections.

%TODO do something with this: Hajek has shown that an analogue of this is true in the context of fuzzy logic \cite{hajek98}.

To put our work in context, we will use the following two factor %TODO maybe change
...
%TODO huh?? \footnote{Note that Leitgeb's axioms tie together his inner notions of truth and probability in a manner that makes it difficult to create from his theories a theory with just an inner probability function.}% quote his reflection principle
...
\end{center}
\end{definition}
%TODO Nate commented that a good way to do this is to pick two things and contrast more specifically, then to note that similar work to A, B, C, etc.
...
%TODO Do we include this??? Unclear if we want it
%Our work can also be situated in the context of autonomous agents. Agents which can model their own reasoning systems may be necessary as a step towards fully self-modifying reasoners (such as those discussed in Soares et al.\cite{soares14}).
%Inner probability functions that approximate the agent's outer probabilities sufficiently well may allow an agent to do just this. For example, an agent that reasons using the outer probability function $\PP$ might be able to prove useful things about its own reasoning system by proving things about $P$.
%As Gaifman\cite{gaifman04} points out, the Gaifman coherence conditions can be relaxed, making them useful for modeling bounded probabilistic agents. This gives a clear path forward for a future extension of our work to the bounded case.

\subsection{Structure of the Paper}
%TODO [intro-sec-norms] check if a norm or not (note: Campbell-moore seems to do this, but she may not be representative)
...
%TODO [prob-log] Finish probabilistic stuff from [prob-log] 

%%fakesubsection

%TODO rewrite pre-intuitions part to be more sensible and motivated for average reader. Talk about how we dont really want to talk about truth here, but rather about probabilistic valuation. This is how/why eq \ref{truth-predicate-reflection} makes sense as the first thing to try.
%TODO It seems like this requires the probabilistic research from the previous section
%TODO [prob-log] Finish probabilistic stuff from [prob-log] 
...
Like how the predicate $True$ attempted to satisfy the reflection principle

\begin{equation}
\vp \iff True(\q{\vp}),
\end{equation}

we might analogously want the function symbol $P$ to satisfy the reflection principle

\begin{equation}\label{truth-predicate-reflection}
\PP(\vp)=p \iff \PP(\Pq{\vp}=p) = 1 .
\end{equation}

But this falls immediately to the liar's sentence $\vp \iff \Pq{\vp} < 0.5$ (``This sentence is unlikely'').

We deal with the liar's paradox by loosening our reflection principle: not requiring $P$ to so closely reflect $\PP$.
This is a flexibility that the probabilistic framework affords us. Pure truth would not.
Our approach is similar to Hajek's \cite{hajek98}: we deny $P$ access to the exact values of $\PP$ but instead let $P$ have \emph{arbitrarily precise} information about $\PP$.
...
But this unfortunately falls to the same liar's sentence ($\vp \iff \Pq{\vp} < 0.5$). The issue with (\ref{literal-reflection}) turns out to be its biconditional nature. Replacing the biconditional with a one directional implication yields a reflection principle that we will prove is consistent.\footnote{Replacing the biconditional with a conditional in our reflection principle mirrors Scharp's distinction between ascending and descending truth\cite{scharp13}.}

%TODO add bit about how to get to next stage

\begin{definition}
\label{refl-def}
\definitionsOfLsAndTFirst
Let $\PP$ be a \gcoh probability distribution over $L'$. We say that $\PP$ is \emph{reflective} iff it satisfies the following reflection principle:
\begin{equation}\label{reflection}
\forall \vp \in L' \; \forall a, b \in \mathbb{Q} : \inOpen{\Po{\vp}} \Longrightarrow \Po{\inOpen{\Pq{\vp}}} = 1 .\\
~\footnote{Note that the reverse implication is necessarily false (by the arguments above against biconditionals); however, by applying the contrapositive twice, it can be shown that condition~\ref{reflection} is equivalent to
$$\forall \vp \in L' \; \forall a, b \in \mathbb{Q} : \inClosed{\Po{\vp}} \Longleftarrow \Po{\inClosed{\Pq{\vp}}} > 0$$
Note that this is almost the reverse implication, except for the fact that it uses closed instead of open intervals and ends with $>0$ instead of $=1$.
}
\end{equation}
\end{definition}

Our goal is to show that starting from any (rich enough\footnote{That is, it admits a G\"odel numbering.}) consistent theory $T$
we can obtain a \emph{\coh}, \emph{reflective} $\PP$.

The intuition behind the name ``reflective''
is that there is a relation, which corresponds to a restricted version of the intuitive concept of belief, such that reflective $\PP$s are exactly those $\PP$s that `believe' in themselves.
This is analogous to the standard view of truth
as a fixed point of a certain revision operation, as in Kripke's landmark paper\cite{kripke75}.
Indeed, we show that reflective $\PP$s exist
by constructing the belief relation and appealing
to Kakutani's fixed point theorem.
\begin{theorem} \emph{Consistency of the Reflection Principle}: 
\definitionsOfLsAndT
There exists a \gcoh $\PP$ which satisfies the reflection principle:
$$\qquad \qquad \forall \vp \in L' \ \forall a,b\in \mathbb{Q}: \inOpen{\Po{\vp}} \Rightarrow \PP(\inOpen{\Pq{\vp}})=1.$$
\end{theorem}
\begin{proof}
Let us define a relation $R$ which will correspond to the notion that a probability distribution $x$ is `believed' by another probability distribution $y$.
$$xRy \iff \forall \vp \in L' \ \forall a, b \in \mathbb{Q}: \inOpen{x(\vp)} \Rightarrow y(\inOpen{\Pq{\vp}})=1$$
Intuitively, a reflective $\PP$ is just a $\PP$ that 'believes' in itself. That is, an $\PP$ such that $\PP R \PP$.
...
Kakutani's fixed point theorem states that there exists $x \in \mathcal{A}$ such that $xRx$, given that the following hold\cite{kakutani41}:
\begin{enumerate}
...
It is easy to see that $x$ is \gcoh and thus an element of $\mathcal{A}$.
...
\item \emph{$\forall x \exists y: xRy$.}
...
Since $x$ is \gcoh, this theory is consistent.
Let $y$ assign $1$ to all statements in $T^*$ and $0$ to all statements not in $T^*$.
This $y$ satisfies $xRy$ and is \gcoh.
...
Therefore, by Kakutani's fixed point theorem, $R$ has a fixed point, which is, by definition, a \coh distribution that satisfies the reflection principle.
\end{proof}
...
The reflection principle for truth predicates is a statement in the \emph{metalanguage}:
$$\vp \iff \Tq{\vp}.$$
If one had a working truth predicate, a natural next desidaratum is for the theory to \emph{know} that the truth predicate works as a truth predicate. Specifically, we might want the theory to include as a \emph{sentence in the language}
$$\vp \leftrightarrow \Tq{\vp}.$$
Note the difference between $\leftrightarrow$, biconditional in the language, and $\iff$, biconditional in the metalanguage.

Analogously, now that we have proven that $P$ works like a truth predicate, we might additionally want $\PP$ to \emph{believe} that $P$ works like a truth predicate. That is, we would like $\PP$ to assign high probability to the statement that $P$ is reflective and \gcoh.

We will show that, unfortunately, no reflective, \coh $\PP$ believes that $P$ is reflective and that any \coh $\PP$ which does believe that $P$ is reflective is not itself reflective.
...
To this end, define the \emph{inner reflection principle} as:
$$\forall\vp\in L' \ \forall a,b\in\mathbb{Q}:\PP(\Pq{\vp}\in(a,b)\rightarrow\Pq{\Pq{\vp}\in(a,b)}=1)=1.~\footnote{Note that the quantification here is in the metalanguage. An even stronger inner reflection principle would have the quantification on the inside i.e.
$$\forall\vp\in L' : \PP(\forall a,b\in\mathbb{Q} : \Pq{\vp}\in(a,b)\rightarrow\Pq{\Pq{\vp}\in(a,b)}=1)=1$$
Or even
$$\PP(\forall\q{\vp} \ \forall a,b\in\mathbb{Q} : \Pq{\vp}\in(a,b)\rightarrow\Pq{\Pq{\vp}\in(a,b)}=1)=1$$ (where $\forall \q{\vp}$ is quantifying over all (valid) encodings in the language, which might be, for example, $\mathbb{N}$.) The impossibility results in this section are proved for the weak version, so carry for these two stronger versions trivially.}}$$
We will first prove that there exist \emph{\coh} inner reflective $\PP$s, but we then prove that these inner reflective $\PP$s cannot also be outer reflective.

\begin{proposition} \emph{Existence of an Inner Reflective $\PP$.}
\definitionsOfLsAndT Then there exists a \gcoh $\PP$ which satisfies the inner reflection principle:
...
\definitionsOfLsAndT
Then there exists no \gcoh $\PP$ which assigns probability one to every sentence in $T$ and satisfies both the inner and outer reflection principles. In fact, any \coh, reflective $\PP$ must assign probability $0$ to inner reflection.
\end{proposition}
\begin{proof}
...
A similar argument shows that any \coh, reflective $\PP$ must assign probability $0$ to inner reflection.
\end{proof}
Not all is lost. We can show that every \coh, reflective $\PP$ satisfies a weaker version of the inner reflection principle.
\begin{proposition} \emph{Weaker Inner Reflection.}
Every \gcoh, reflective $\PP$ satisfies
...
We would also like $\PP$ to assign probability $1$ to $P$ being \gcoh. Here, the situation is much better. We will prove that there exist $\PP$'s which do exactly this.
Let $Coh(P)$ be the sentence stating that $P$ is \gcoh. Specifically, $Coh(P)$ is
$$\forall \vp: (\Pq{\vp} \in [0, 1]) \wedge (\vp \rightarrow \Pq{\vp}=1) \wedge (\forall \psi: \Pq{\vp} = \Pq{\vp \wedge \psi} + \Pq{\vp \wedge \neg \psi})$$
\begin{proposition}
There exists a \coh, reflective $\PP$ such that $\PP(Coh(P))=1$.
\end{proposition}
\begin{proof}
...
%TODO MATH sidenote: all the proofs seem to follow the same way. Can we look at the space of all things that we can prove in this way in this framework? What is the maximum result strength?
\end{proof}
...
\begin{proposition}
There does not exist a \coh, reflective $\PP$ and an $\varepsilon$ such that
$$\forall a,b \in\mathbb{Q}\forall\vp\in L':\PP(Coh(P)\wedge(\Pq{\vp}\in[a,b]\rightarrow\Pq{\Pq{\vp}\in(a,b))}=1)>\varepsilon$$
\end{proposition}
\begin{proof}
Suppose such a \coh $\PP$ and $\varepsilon$ exist.
...
\label{refl-exp}
%TODO check the exposition in this section
%TODO looks like motivation and stuff may need serious work still
...
%TODO rafael and which assigns probability~1 to the proposition that $P$ is a \coh probability distribution which itself assigns probability~1 to the same proposition. 
...
%%fakesubsection
Having built up some machinery in the previous section, we are now ready to prove some results about the reflection principle and standard models of PA. We prove that reflective, \coh $\PP$'s  assign zero probability to standard models. We take this as further evidence that a weaker reflection principle is needed.%TODO needed for what??
\subsection{Assigning Probabilities to Sets of Standard Theories of PA}
The following definition is different from our standard definition of $P$. Since we are now working in the realm of Peano arithmetic, we can weaken our definition of our inner probability distribution. Instead of defining it as a function in the language, we will show that it is sufficient for it to be a three place relation which arbitrarily approximates a metalanguage function.
\begin{definition}
Let $L'$ be the extension of the language $L$ (which contains at least the language of arithmetic) by a relation symbol $P$.
...
\end{definition}
Intuitively, $P$ is a three-valued relation which approximates some function $f$ arbitrarily well. We imagine $(\q{\vp},a,b)\in P$ to mean that $a<f(\vp)<b$, with f being the function that we require to exista
...
%TODO motivate this as we tried the obvious paradox and it output this. Change the description to something more philosophy?
%TODO needs intro stuff done (prob-log)
...
\label{further-work}
%TODO check this section. Maybe rewrite to be more in vein of new intro. (bottleneck: [prob-log])
...
so our result implies that there exist \coh distributions satisfying this principle.
...
The authors would like to thank Nate Soares, Patrick LaVictoire, Kenny Easwaran, and Giego Caleiro for their help with this paper.
