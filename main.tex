...
%TODO change the terminology of inner probability function to something saner and more meaningful and less confusing
%TODO talk to Steve about this section especially. And to others with relevant expertise. [already emailed Patrick]
...
Our work is novel in having outer notions 
of truth and probability as well as an inner probability function
...
In section~\ref{refl-exp}, we prove %TODO word!!!!
...
But, this falls immediately to the liar's sentence $G \leftrightarrow \Pq{G} < 0.5$ (``This sentence is unlikely'')\footnote{A sketch of the proof that satisfying \ref{truth-predicate-reflection} leads to contradiction:

Assume $\PP(\vp)=p \iff \PP(\Pq{\vp}=p) = 1$.
Then, we can use the diagonal lemma to construct the sentence (in the language) $G \leftrightarrow \Pq{G} < 0.5$.
We show that $\PP(G)=0.5$ with a double proof by contradiction:

Say $\PP(G)<0.5$. 
By reflection, we have $\PP(\Pq{G}<0.5)=1$.
But, (by coherence) this is equivalent to $\PP(G)=1$, a contradiction.

A similar argument proves that $\PP(G)$ cannot be greater than $0.5$.
...
$$\forall \vp \in L' \; \forall a, b \in \mathbb{Q} : \inClosed{\Po{\vp}} \Longleftarrow \Po{\inClosed{\Pq{\vp}}} > 0.$$ We use this formulation in some of the proofs in section~\ref{refl-refl}.
...
\item \emph{$\forall p\in\mathcal{A} \exists q\in\mathcal{A}: pRq$.}
Fix a $p$. Take a completion $T^*$ which, whenever $\inOpen{p(\vp)}$, includes the statement $\inOpen{\Pq{\vp}}$.
...
...
It is derived analogously to the reverse outer reflection principle (...).
%TODO MATH
}
...
%TODO MATH sidenote: all the proofs seem to follow the same way. Can we look at the space of all things that we can prove in this way in this framework? What is the maximum result strength? reverse math it or something??
\end{proof}
...
\begin{proposition}
There does not exist a coherent, reflective probability distribution $\PP$ and an $\varepsilon\in\mathbb{R}$ such that
$$\forall a,b \in\mathbb{Q}\forall\vp\in L':\PP(Coh(P)\wedge(\Pq{\vp}\in[a,b]\rightarrow\Pq{\Pq{\vp}\in(a,b))}=1)>\varepsilon$$
\end{proposition}
\begin{proof}
%TODO MATH fix this to not be bs
We proceed with a proof by contradiction.
...
$$\PP(\bot)>0$$
%TODO MATH fix notation and motivate steps better
which contradicts $\PP$’s coherence. Therefore, $\PP(\Pq{\vp}<1-\varepsilon\wedge IRP)=0$.
